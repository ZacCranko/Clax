...
class TrainIterator:
  ...
  def __init__(
      self,
      dataset_builder: tf.data.Dataset,
      dataset_iter,
      batch_size: int,
      split: str = "train",
      num_steps: int = -1,
    ...
    if num_steps <= 0 and num_epochs <= 0:
      raise ValueError(
          "Must supply a positive number of either steps or epochs")
    ...
    self.num_steps = (num_steps if num_steps > 0 else int(num_epochs *
                                                          self.steps_per_epoch))
    self.start_step = start_step

    if self.start_step >= self.num_steps:
      raise ValueError(
          f"Must have start_step (got {self.start_step}) earlier than num_step (got {self.num_steps})"
      )
  ...
  def is_freq(self,
    ...
    if force_last and (self.global_step + 1 == self.num_steps):
      ...
      raise ValueError(
          f"Must supply positive step_freq (got {step_freq}) or epoch_freq (got {epoch_freq})"
      )
