...
class CLTrainState(train_state.TrainState):
  ...
  def encode(self, x, train: bool = False):
    params = dict(params=self.params["backbone"],
                  batch_stats=self.batch_stats["backbone"])
    return self.backbone_apply_fn(params, x, train=train, mutable=False)

  def classify(self, encodings, train: bool = False):
    return self.clf_heads_apply_fn(dict(params=self.clf_heads_params),
                                   encodings)
...
def initialise_assembly(key: jnp.ndarray, image_shape: Tuple[int, int, int],
                        model: Module):
...
def initialise_linear_clf_heads(key: ndarray,
                                input_dim: int,
                                num_classes: int,
                                num_heads: int = 10):
  clf = models.classifier.MutiHeadClassifier(num_heads=num_heads,
                                             num_classes=num_classes)
  params = clf.init(key, jnp.ones((0, input_dim)))
  return clf.apply, params


def create_train_state(key, config: ConfigDict, assembly, image_shape,
                       learning_rate_fn, workdir: str) -> CLTrainState:
  ...
  clf_heads_apply_fn, clf_heads_params = initialise_linear_clf_heads(
      key, input_dim=2048, num_classes=10, num_heads=1)
  ...
  state = CLTrainState.create(apply_fn=assembly.apply,
  ...
  if config.restore_projector != "":
    ...
    checkpoint_path = (stored_run if os.path.isdir(stored_run) else
                       os.path.join(workdir, "..", stored_run))
