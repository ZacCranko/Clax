...
def train_and_evaluate(config: ConfigDict, workdir: str) -> TrainState:
  ...
  train_iter = data.create_input_iter(config, num_transforms = 2)
  ...
  linear_train_iter = data.create_input_iter(config.clf_config, dataset = config.dataset, num_transforms = 1)
