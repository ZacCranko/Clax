...
import data
import models, objective as obj
...
def create_train_state(
    rng: jnp.ndarray, clf_config: ConfigDict, state, assembly, train_iter: TrainIterator
):
    ...
    _, num_features = encod_state.apply(
        jnp.ones((128,) + train_iter.image_shape), train=False
    ).shape

    # initialise model
    clf, params = initialized(
        rng, train_iter.num_classes, num_features, clf_config.num_heads
    )
...
def train_step(
    ...
    def loss_fn(params):
        ...
        cross_entropy = (
            v_softmax_cross_entropy(logits_by_head, labels_by_head).mean(-1).sum()
        )
...
def compute_metrics(
    rep_encod_state: train_state.TrainState,
...
def train_and_evaluate(
    encod_state: train_state.TrainState,
    clf_state: train_state.TrainState,
    train_iter: data.TrainIterator,
    test_iter: data.TrainIterator,
    l2coeffs,
    debug: bool = False,
):
    ...
    for batch in train_iter:
        ...
        if train_iter.is_epoch_end() and debug:
            metrics_str = " ".join(
                f"{metric}: {val:.2e}"
                for metric, val in jax_utils.unreplicate(metrics).items()
            )
...
def linear_accuracy(
    ...
    encod_state, clf_state = create_train_state(
        subkey, config.clf_config, state, assembly, train_iter
    )

    l2coeffs = jnp.linspace(minl2coeff, maxl2coeff, num_heads)

    logging.info("Training classifier heads")
    test_metrics, clf_state = train_and_evaluate(
        encod_state, clf_state, train_iter, test_iter, l2coeffs, debug=debug
    )
