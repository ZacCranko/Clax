...
def multinomial_logistic_loss(params, embeddings, labels, mask, num_replicas,
                              l2_regularization):
...
def _lbfgs_minimize(embeddings, labels, mask, initial_position, loss,
  ...
  return tfp.optimizer.lbfgs_minimize(total_loss_and_grad_fn,
...
def train(embeddings,
  ...
  initial_position = weights_and_biases_to_params(initial_weights,
                                                  initial_biases)

  res = _lbfgs_minimize(embeddings, labels, mask, initial_position, loss,
                        lbfgs_args)
...
@functools.partial(jax.pmap, axis_name='batch', in_axes=(0, 0, 0, None, None))
def _evaluate(embeddings, labels, mask, weights, biases):
  """Function to be pmapped to perform evaluation."""
  return (jnp.sum(
      (jnp.argmax(embeddings.dot(weights) + biases[None, :], -1) == labels) *
      mask), jnp.sum(mask))
...
def tune_l2_regularization(train_embeddings,
                           train_labels,
                           train_mask,
                           val_embeddings,
                           val_labels,
  ...
  for l2 in initial_range:
    print(l2)
    weights, biases, _ = train(train_embeddings,
                               train_labels,
                               train_mask,
                               l2,
                               weights,
                               biases,
                               loss=loss)
    accuracy = evaluate(val_embeddings, val_labels, val_mask, weights, biases)
    print(f'  {accuracy}')
  ...
  for _ in range(num_steps):
    ...
    if accuracy > best_accuracy:
      best_l2, best_weights, best_biases, best_accuracy = (l2, weights, biases,
                                                           accuracy)
      continue
